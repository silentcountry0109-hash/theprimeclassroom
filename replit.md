...
- **Booking Status Flow**: Bookings follow the status flow: `confirmed` → `checked_in` → `completed`. Coaches can check-in students via `PATCH /api/coach/bookings/:id/check-in`. `checked_in` status means the student is attending class ("上課中"). Both `confirmed` and `checked_in` auto-complete when the slot date passes. Parents see "上課中" badge on checked-in bookings. Franchise admin stat card shows "已上課" (completed + checked_in count).
