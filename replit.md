...
- **Coach Compensation System**: Each coach has a configurable compensation type: "fixed" (per-lesson fixed amount) or "percentage" (percentage of lesson revenue based on actual collected amount). Franchise admins set compensation via the coach edit dialog. Coaches see their compensation method, confirmed income, and projected monthly income in the "收入總覽" tab. API: `GET /api/coach/earnings`, `GET /api/franchise-admin/coach-earnings`. DB fields: `coaches.compensation_type`, `coaches.compensation_amount`. Server-side validation enforces type enum, non-negative amounts, and percentage ≤100.
