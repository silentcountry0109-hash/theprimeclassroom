...
- **Student Code (學生編碼)**: Each child gets a unique `studentCode` in format `YYYYMMDD-XXXX` (Taiwan date + 4-digit sequential). Auto-generated on creation via `generateStudentCode()` in `server/storage.ts`. Displayed on parent dashboard child cards and coach dashboard student lists. Existing children are backfilled on server startup.
